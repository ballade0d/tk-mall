...
func (uq *UserQuery) QueryPassword() *PasswordQuery {
...
func (uq *UserQuery) IDs(ctx context.Context) (ids []int32, err error) {
...
func (uq *UserQuery) WithPassword(opts ...func(*PasswordQuery)) *UserQuery {
	query := (&PasswordClient{config: uq.config}).Query()
...
func (uq *UserQuery) loadPassword(ctx context.Context, query *PasswordQuery, nodes []*User, init func(*User), assign func(*User, *Password)) error {
	...
	for _, n := range neighbors {
		fk := n.user_password
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_password" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(user.Table, user.Columns, sqlgraph.NewFieldSpec(user.FieldID, field.TypeInt32))
	_spec.From = uq.sql
