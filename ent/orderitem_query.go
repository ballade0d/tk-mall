...
func (oiq *OrderItemQuery) QueryItem() *ItemQuery {
	query := (&ItemClient{config: oiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oiq *OrderItemQuery) WithItem(opts ...func(*ItemQuery)) *OrderItemQuery {
	query := (&ItemClient{config: oiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (oiq *OrderItemQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderItem, error) {
	...
	if oiq.withOrder != nil || oiq.withItem != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := oiq.withItem; query != nil {
		if err := oiq.loadItem(ctx, query, nodes, nil,
			func(n *OrderItem, e *Item) { n.Edges.Item = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oiq *OrderItemQuery) loadItem(ctx context.Context, query *ItemQuery, nodes []*OrderItem, init func(*OrderItem), assign func(*OrderItem, *Item)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OrderItem)
	for i := range nodes {
		if nodes[i].order_item_item == nil {
			continue
		}
		fk := *nodes[i].order_item_item
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "order_item_item" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
