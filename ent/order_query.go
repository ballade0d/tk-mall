...
func (oq *OrderQuery) QueryPayment() *PaymentQuery {
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withPayment: oq.withPayment.Clone(),
...
func (oq *OrderQuery) WithPayment(opts ...func(*PaymentQuery)) *OrderQuery {
	query := (&PaymentClient{config: oq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	oq.withPayment = query
	return oq
}
...
func (oq *OrderQuery) loadPayment(ctx context.Context, query *PaymentQuery, nodes []*Order, init func(*Order), assign func(*Order, *Payment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Payment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.PaymentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_payment
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_payment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_payment" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
